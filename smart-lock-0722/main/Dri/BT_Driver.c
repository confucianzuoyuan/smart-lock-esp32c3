...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event,
  ...
  case ESP_GATTS_WRITE_EVT: {
    ...
    if (!param->write.is_prep) {
      // param->write.len手机蓝牙调试助手发送给esp32的数据的长度
      ESP_LOGI(GATTS_TAG,
               "GATT_WRITE_EVT, value len %d, value :", param->write.len);
      // esp32接收到的数据param->write.value
      ...
      if (!memcmp(param->write.value, "openlock", 8)) {
        printf("open lock success!\r\n");
        MOTOR_OpenLock();
      }

      // 通过蓝牙修改并保存密码
      // addpassword+123456
      if (!memcmp(param->write.value, "addpassword+", 12)) {
        // 解析密码
        int password = (param->write.value[12] - '0') * 100000 +
                       (param->write.value[13] - '0') * 10000 +
                       (param->write.value[14] - '0') * 1000 +
                       (param->write.value[15] - '0') * 100 +
                       (param->write.value[16] - '0') * 10 +
                       (param->write.value[17] - '0') * 1;
        // 将密码存储到nvs flash中
